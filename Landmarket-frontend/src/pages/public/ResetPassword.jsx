            ...
            <div className="auth-form-side" style={{ width: '100%', maxWidth: '100%', alignItems: 'center', justifyContent: 'center' }}>
                ...
                <div className="auth-form-container" style={{ maxWidth: '400px', width: '100%', margin: '0 auto', background: 'rgba(255,255,255,0.02)', padding: '2rem', borderRadius: '1rem', border: '1px solid rgba(255,255,255,0.05)' }}>
                    <div className="auth-header text-center mb-8">
                        ...
                        <p className="text-muted text-sm">Enter your token and new password</p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="auth-form space-y-4">
                        <div className="auth-input-group">
                            <label className="auth-label block mb-2 text-xs font-bold uppercase tracking-widest text-muted">Recovery Token</label>
                            <input
                                type="text"
                                placeholder="Paste the token from server console here"
                                required
                                value={token}
                                onChange={(e) => setToken(e.target.value)}
                                className="auth-input w-full bg-white/5 border border-white/10 rounded-xl p-3 text-white focus:border-primary/50 transition-all font-mono text-xs"
                            />
                        </div>

                        <div className="auth-input-group mt-4">
                            <label className="auth-label block mb-2 text-xs font-bold uppercase tracking-widest text-muted">New Password</label>
                            <input
                                type="password"
                                placeholder="••••••••••••"
                                required
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="auth-submit-btn w-full bg-primary text-white font-black uppercase tracking-widest py-3 rounded-xl hover:bg-primary/90 transition-colors disabled:opacity-50 mt-6 block"
                        >
                    ...
                    <p className="auth-footer-text text-center mt-6 text-sm text-muted">
                        Back to <Link to="/login" className="text-primary hover:text-white transition-colors">Sign In</Link>
                    </p>
                </div>
            </div>
        </div>
